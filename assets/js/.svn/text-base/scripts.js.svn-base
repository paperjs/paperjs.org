...
function createCodeMirror(place, options, source) {
	...
	var text2 = text.replace(/\t/gi, '    ').match(
...
PaperScript = HtmlElement.extend({
	...
	initialize: function() {
		...
		if (explain) {
			...
			explain = button.injectAfter('div', {
				className: 'explain hidden'
			});
			var timer,
				mode;
			animateExplain = function(clearPrevious) {
				...
				if (entry) {
					...
					if (entry[1])
						explain.setStyle('margin-bottom', -explain.getHeight() + 'px');
					timer = (function() {
			...
			this.addEvents({
				...
				mouseout: function(event) {
					...
					if (timer && button.getStyle('display') == 'none') {
						timer = timer.clear();
						explain.addClass('hidden');
					}
				}
